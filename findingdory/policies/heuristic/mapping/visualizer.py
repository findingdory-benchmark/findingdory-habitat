...
import findingdory.policies.heuristic.mapping.utils.visualization_utils as vu
...
from .constants import SemanticCategoryMapping
...
class Visualizer:
    ...
    def visualize(
        ...
        semantic_category_mapping: Optional[SemanticCategoryMapping] = None,
        ...
        **kwargs,
        ...
        if obstacle_map is not None:
            ...
            # Create obstacle map visualization
            obstacle_map_vis = np.zeros((*obstacle_map.shape, 3), dtype=np.uint8)
            obstacle_mask = np.rint(obstacle_map) == 1
            obstacle_map_vis[obstacle_mask] = [0, 0, 255]  # Red for obstacles
            obstacle_map_vis[~obstacle_mask] = [255, 255, 255]  # White for free space
            
            # Resize to match the display size
            obstacle_map_vis = cv2.resize(
                obstacle_map_vis,
                (V.TOP_DOWN_W, V.HEIGHT),
                interpolation=cv2.INTER_NEAREST,
            )
            
            # Add agent arrow to obstacle map visualization
            pos = (
            ...
            # Use (0, 0) as origin for the standalone obstacle map
            agent_arrow_for_obs_map = vu.get_contour_points(pos, origin=(0, 0))
            color = (255, 0, 0)  # Green arrow for visibility
            obstacle_map_vis_with_arrow = obstacle_map_vis.copy()
            cv2.drawContours(obstacle_map_vis_with_arrow, [agent_arrow_for_obs_map], 0, color, -1)
            
            # Keep the original arrow for the main visualization
            ...
            final_sem_map_vis = image_vis[V.Y1 : V.Y2, V.TOP_DOWN_X1 : V.TOP_DOWN_X2]
        ...
        final_first_person_sem_frame = image_vis[V.Y1 : V.Y2, V.FIRST_SEM_X1 : V.FIRST_SEM_X2]
        ...
        return final_sem_map_vis, final_first_person_sem_frame, obstacle_map_vis_with_arrow
