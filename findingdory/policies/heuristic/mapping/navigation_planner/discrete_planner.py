...
from findingdory.policies.heuristic.mapping.interfaces import (
    ContinuousNavigationAction,
    DiscreteNavigationAction,
)
from findingdory.policies.heuristic.mapping.utils.geometry import xyt_global_to_base
...
class DiscretePlanner:
    ...
    def __init__(
        ...
        dump_location: str,
        exp_name: str,
        dilate_obstacles: bool,
        min_goal_distance_cm: float,
        ...
        Arguments:
            ...
            dilate_obstacles: if True, dilate obstacles for planning
        """
    ...
    def plan(
        ...
        if replan and not stop:
            # Return STOP immediately without replanning
            return (
                DiscreteNavigationAction.STOP,
                np.zeros_like(goal_map),  # Empty closest_goal_map
                (0, 0),  # Empty short_term_goal
                np.zeros_like(obstacle_map),  # Empty dilated_obstacles
                False, # Was planner successful ?
            )
            
            # The code below will no longer execute
            # Clean collision map
        ...
        return action, closest_goal_map, short_term_goal, dilated_obstacles, True
