...
from findingdory.policies.heuristic.mapping.modules.semantic.categorical_2d_semantic_map_module import (
    Categorical2DSemanticMapModule,
)
from findingdory.policies.heuristic.mapping.navigation_policy.objectnav_frontier_exploration_policy import (
    ObjectNavFrontierExplorationPolicy,
)
from findingdory.policies.heuristic.mapping.navigation_policy.global_pointnav_policy import (
    GlobalPointNavigationPolicy,
)
...
class SemanticMapperAgentModule(nn.Module):
    def __init__(self, config, instance_memory: Optional[InstanceMemory] = None):
        ...
        #     ),
        # )
        self.policy = GlobalPointNavigationPolicy()
    ...
    def forward(
        ...
        if isinstance(self.policy, GlobalPointNavigationPolicy):
            if goal_global_pose is not None:
                goal_map = self.policy(
                    final_global_map,
                    goal_global_pose,
                )
                found_goal = True
            else:       
                # NOTE: Can pass anything here as the goal map is only used in the planner when the experience collection is completed
                goal_map = torch.zeros_like(final_global_map[0,0]).unsqueeze(0).unsqueeze(0)
                found_goal = False
            found_goal = torch.Tensor([found_goal])
        else:    
        ...
        if isinstance(self.policy, GlobalPointNavigationPolicy):
            frontier_map = torch.zeros_like(torch.zeros_like(final_global_map[0,0])).unsqueeze(0).unsqueeze(0)
        else:
