...
import findingdory.policies.heuristic.mapping.modules.map_utils as mu
import findingdory.policies.heuristic.mapping.utils.depth as du

import findingdory.policies.heuristic.mapping.utils.pose_utils as pu
import findingdory.policies.heuristic.mapping.utils.rotation as ru
...
from findingdory.policies.heuristic.mapping.utils.spot import draw_circle_segment, fill_convex_hull
...
class Categorical2DSemanticMapModule(nn.Module):
    ...
    def _update_local_map_and_pose(  # noqa: C901
        ...
        if camera_pose is not None:
            ...
            angles = torch.Tensor(
                [tra.euler_from_matrix(p[:3, :3].cpu(), "ryzx") for p in camera_pose]
            ).to(device)
            
            # Extract roll, tilt, yaw angles from the angles tensor
            # In "ryzx" convention: angles[i] = [roll, yaw, tilt]
            roll = angles[:, 0]
            yaw = angles[:, 1]
            tilt = angles[:, 2]
