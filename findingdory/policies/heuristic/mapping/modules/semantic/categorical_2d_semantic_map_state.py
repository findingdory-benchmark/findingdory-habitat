...
from findingdory.policies.heuristic.mapping.modules.map_utils import MapSizeParameters, init_map_and_pose_for_env
...
class Categorical2DSemanticMapState:
    ...
    def get_obstacle_map(self, e, global_map=False) -> np.ndarray:
    ...
    def get_planner_pose_inputs(self, e, global_map_bounds=False) -> np.ndarray:
        ...
        if not global_map_bounds:   
            ...
            return (
                torch.cat([self.local_pose[e] + self.origins[e], torch.Tensor([0, self.global_map_size, 0, self.global_map_size]).to(self.device)])
                .cpu()
                .float()
                .numpy()
            )
