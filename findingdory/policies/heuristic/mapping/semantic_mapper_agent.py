...
import findingdory.policies.heuristic.mapping.utils.pose_utils as pu
from .interfaces import DiscreteNavigationAction, Observations
from findingdory.policies.heuristic.mapping.modules.instances.instance_map import InstanceMemory
from findingdory.policies.heuristic.mapping.modules.semantic.categorical_2d_semantic_map_state import (
    Categorical2DSemanticMapState,
)
from findingdory.policies.heuristic.mapping.navigation_policy.global_pointnav_policy import GlobalPointNavigationPolicy

from .semantic_mapper_agent_module import SemanticMapperAgentModule
from findingdory.policies.heuristic.mapping.navigation_planner.discrete_planner import DiscretePlanner
...
class SemanticMapperAgent:
    ...
    def __init__(
        ...
        self.panorama_start_steps = 0       # We will already have a populated map from Phase 1 experience collection so panaroma routine is not needed
        ...
        self._module = SemanticMapperAgentModule(
        ...
        if isinstance(self._module.policy, GlobalPointNavigationPolicy):
            self.semantic_map.goal_map = np.zeros(
                (self.semantic_map.num_environments, self.semantic_map.global_map_size, self.semantic_map.global_map_size)
            )
            self.semantic_map.frontier_map = np.zeros(
                (self.semantic_map.num_environments, self.semantic_map.global_map_size, self.semantic_map.global_map_size)
            )
        ...
        self.planner = DiscretePlanner(
            ...
            dilate_obstacles=config.AGENT.PLANNER.dilate_obstacles,
        ...
        self.verbose_planner = config.AGENT.PLANNER.verbose
    ...
    def prepare_planner_inputs(
        ...
        (
            ...
            goal_global_pose=goal_global_pose,
        )
        ...
        if isinstance(self.module.module.policy, GlobalPointNavigationPolicy):
            planner_inputs = [
                {
                    "obstacle_map": self.semantic_map.get_obstacle_map(e, global_map=True),
                    "goal_map": self.semantic_map.get_goal_map(e),      # This stores the goal map from global point navigation policy
                    "frontier_map": self.semantic_map.get_frontier_map(e),
                    "sensor_pose": self.semantic_map.get_planner_pose_inputs(e, global_map_bounds=True),
    ...
    def act(self,
            ...
            goal_global_pose=None,
            obstacle_map_for_planner=None) -> Tuple[DiscreteNavigationAction, Dict[str, Any]]:
        """Act end-to-end."""
        if self.get_timing:
            t0 = time.time()
        ...
        if goal_global_pose is not None:
            goal_global_pose = (goal_global_pose[:,:2] * 100. / self.semantic_map.map_size_parameters.resolution).int()
        ...
        if can_take_action:
            # We override the latest updated obstacle map with the obstacle map from the end point of experience collection as there are no new updates to the obstacle map thereafter
            # This helps prevent planner failures due to smeared out obstacle map
            assert obstacle_map_for_planner is not None, "Obstacle Map for Planner is Not Provided !"
            planner_inputs[0]["obstacle_map"] = obstacle_map_for_planner
        ...
        return action, info, self.semantic_map.global_pose, self.semantic_map.get_obstacle_map(0, global_map=True), planner_success
