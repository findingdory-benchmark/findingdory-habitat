...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def __init__(
        ...
        if self.classifier_feature == "reshape_embedding":
            self.final_spatial = self.patch_embed.grid_size
            self.embed_dim = (
...
def reshape_embedding(x, final_spatial):
    N, _, D = x.shape
    H, W = final_spatial
    x = x.reshape(N, H, W, D)
...
def load_mae_encoder(model, checkpoint_path=None):
    ...
    if checkpoint_path is None:
        ...
        checkpoint_path = model_utils.download_model_if_needed(checkpoint_path)
