...
class AgentStateSpace(spaces.Space):
    """Custom space for AgentState observations"""
    def __init__(self):
        super().__init__(shape=None, dtype=np.float32)
        # Position space: 3D vector
        ...
        # Rotation space: 4D quaternion
        self.rotation_space = spaces.Box(
            low=-1.0,
            high=1.0,
            shape=(4,),
            dtype=np.float32
    ...
    def sample(self):
        return AgentState(
            position=self.position_space.sample(),
            rotation=self.rotation_space.sample()
        )

    def contains(self, x):
        if not isinstance(x, AgentState):
            return False
        return (self.position_space.contains(x.position) and 
                self.rotation_space.contains(x.rotation))
        
class KeyframesStateSpace(spaces.Space):
    """Custom space for keyframe observations that represent a list of integers"""
    def __init__(self):
        super().__init__(shape=None, dtype=object)
        # We use object dtype since we're dealing with variable-length lists

    def sample(self):
        # Return an empty list as a sample
        # You could modify this if you want to generate random valid keyframes
        return []

    def contains(self, x):
        # Check if x is a list and all elements are integers
