...
def transform_augment(
    ...
    if type(resize_size) == omegaconf.listconfig.ListConfig:
        resize_size = omegaconf.OmegaConf.to_object(resize_size)
    if type(output_size) == omegaconf.listconfig.ListConfig:
        output_size = omegaconf.OmegaConf.to_object(output_size)
    transforms = [
        ToTensorIfNot(),
        T.Resize(
            resize_size, interpolation=T.InterpolationMode.BILINEAR, antialias=True
        ),
